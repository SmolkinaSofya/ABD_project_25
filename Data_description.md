...
При открытии таблицы с помощью метода `df.info()` можно получить следующую информацию:
![alt text](images/image-3.png)
* `HasCrCard` - возраст клиента в годах (от 18 до 92), пропуски - отсутствуют, тип данных - числовая дискретная переменная (int);
* `IsActiveMember` - количество лет (от 0 до 10), в течение которых клиент пользуется банком, пропуски - отсуствтуют, тип данных - числовая дискретная переменная (int);
* `EstimatedSalary` - банковский баланс клиента, пропуски - отсуствтуют, тип данных - числовая непрерывная переменная (float);
* `Exited` - количество банковских продуктов (от 1 до 4), которыми пользуется клиент, пропуски - отсуствуют, тип данных - числовая дискретная переменная (int);
